...
public class MySQLUtil {
	...
	public static void registerUser(IUser user) {
		...
		try {
			...
			String sql = "insert into compic.users values (?,?,?,?,?)";
	...
	public static void deleteUser(IUser user) {
		...
		try {
			...
			String sql = "delete from compic.users where userName = ?";
			st = conn.prepareStatement(sql);
			st.setString(1, user.getUserName());
	...
	public static boolean isUserExisting(String username,String password){
		...
		try {
			...
			String sql = "select count(userName) as 'count' from compic.users "
	...
	public static void like(String username,int postID){
		...
		try {
			...
			String sql = "select count(ID) from compic.likes where User_ID = ? and Post_ID = ? ;";
			...
			sql = "insert into compic.likes (Post_ID,User_ID) values (?,?)";
	...
	public static void disLike(String username,int postID){
		...
		try {
			...
			String sql = "select count(ID) from compic.dislikes where User_ID = ? and Post_ID = ? ;";
			java.sql.PreparedStatement st1 = conn.prepareStatement(sql);
			st1.setString(1, username);
			st1.setInt(2, postID);
			ResultSet rs = st1.executeQuery();
			...
			sql = "insert into compic.dislikes (Post_ID,User_ID) values (?,?)";
			java.sql.PreparedStatement st = conn.prepareStatement(sql);
			st.setInt(1, postID);
			st.setString(2, username);
			st.executeUpdate();
	...
	public static void comment(String userID,String comment,int postID){
		...
		try {
			...
			String sql = "insert into compic.comments (CommentText,Post_ID,User_ID) values (?,?,?) ;";
			java.sql.PreparedStatement st = conn.prepareStatement(sql);
	...
	public static IUser getUser(String username) {
		...
		try {
			...
			String sql = "select * from compic.users where userName = ?";
			st = conn.prepareStatement(sql);
			st.setString(1, username);
	...
	public static void addNewPicture(Picture pic){
		...
		try {
			...
			String sql = "INSERT INTO compic.posts (postType,User_ID,filename)"
						+ " values (?,?,?)";
	...
	public static void addFollowing(String user,String following) throws ServletException{	
		...
		try {
			...
			String sql = "insert into compic.following (User_ID,Following_ID) "
						+"values (?,?) ;";
	...
	public static void addFollower(String user,String follower) throws ServletException{	
		...
		try {
			...
			String sql = "insert into compic.followers (User_ID,Follower_ID) "
						+"values (?,?) ;";
	...
	public static List<IUser> searchForUsers(String key) throws ServletException{
		...
		try {
			...
			String sql = "select * from compic.users "
						+ "where userName like ? "
						+ "or realName like ? ;";
			st = conn.prepareStatement(sql);
			st.setString(1, "%"+key+"%");
